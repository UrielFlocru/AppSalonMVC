<?php
    include_once __DIR__. '/../templates/barra.php';
?>

<h1 class="nombre-pagina"> Crear nueva cita </h1>



<div id="app">
    <nav class="tabs">
        <button type="button" data-step="1">Servicios</button>
        <button type="button" data-step="2">Información Cita</button>
        <button type="button" data-step="3">Resumen</button>
    </nav>

    <div id="step-1" class="seccion">
        <h2>Servicios</h2>
        <p class="text-center">Elige tus servicios</p>
        <div id="services" class="listado-servicios" ></div>
    </div>
    <div id="step-2" class="seccion">
        <h2>Datos y Cita</h2>
        <p class="text-center">Coloca tus datos y fecha de tu cita</p>

        <form class="formulario">
            <div class="campo">
                <label for="name">Nombre</label>
                <input id="name" type="text" placeholder="Tu Nombre" value="<?php echo $name; ?>" disabled>
            </div>
            <div class="campo">
                <label for="date">Fecha</label>
                <input id="date" type="date" min="<?php echo date('Y-m-d', strtotime("+1 day")) ;?>">
            </div>
            <div class="campo">
                <label for="hour">Hora</label>
                <input id="hour" type="time">
            </div>
            <input type="hidden" id="id" value="<?php echo $id; ?>">
        </form>
    </div>
    <div id="step-3" class="seccion contenido-resumen">
        <h2>Resumen</h2>
        <p class="text-center" >Verifica que la información sea correcta</p>
    </div>

    <div class="paginacion">
        <button id="previous" class="boton">&laquo; Anterior </button>
        <button id="next" class="boton">Siguiente &raquo; </button>
    </div>

</div>

<?php 
    $script = "
        <script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script>
        <script src='build/js/app.js'></script>
    ";
?>